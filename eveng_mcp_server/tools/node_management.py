...
def register_node_tools(mcp: "FastMCP", eveng_client: "EVENGClientWrapper") -> None:
    ...
    async def list_node_templates(arguments: ListTemplatesArgs) -> list[TextContent]:
        ...
        try:
            ...
            # Handle SDK returning non-dict or non-dict data
            if not isinstance(templates, dict) or not templates.get('data'):
                return [TextContent(
                    type="text",
            ...
            if not isinstance(templates_data, dict):
                return [TextContent(
                    type="text",
                    text=f"Unexpected template data format: {type(templates_data)}"
                )]
            ...
            for template_name, template_info in templates_data.items():
                if not isinstance(template_info, dict):
                    templates_text += f"📦 {template_name}: {template_info}\n\n"
                    continue
    ...
    async def list_nodes(arguments: ListNodesArgs) -> list[TextContent]:
        ...
        try:
            ...
            if not isinstance(nodes, dict) or not nodes.get('data'):
                return [TextContent(
                    type="text",
            ...
            if not isinstance(nodes_data, dict):
                return [TextContent(
                    type="text",
                    text=f"Unexpected nodes data format: {type(nodes_data)}"
                )]
