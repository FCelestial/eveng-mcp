...
def register_network_tools(mcp: "FastMCP", eveng_client: "EVENGClientWrapper") -> None:
    ...
    async def list_network_types(arguments: ListNetworksArgs) -> list[TextContent]:
        ...
        try:
            ...
            if not isinstance(network_types, dict) or not network_types.get('data'):
                return [TextContent(
                    type="text",
                    text="No network types found on the server."
                )]

            types_data = network_types['data']
            if not isinstance(types_data, dict):
                return [TextContent(
                    type="text",
                    text=f"Unexpected network types data format: {type(types_data)}"
                )]
            ...
            for type_name, type_info in types_data.items():
                if not isinstance(type_info, dict):
                    types_text += f"🌐 {type_name}: {type_info}\n\n"
                    continue
    ...
    async def list_lab_networks(arguments: ListNetworksArgs) -> list[TextContent]:
        ...
        try:
            ...
            if not isinstance(networks, dict) or not networks.get('data'):
                return [TextContent(
                    type="text",
            ...
            if not isinstance(networks_data, dict):
                return [TextContent(
                    type="text",
                    text=f"Unexpected networks data format: {type(networks_data)}"
                )]

            # Format networks information
            networks_text = f"Networks in {arguments.lab_path}:\n\n"

            for net_id, network in networks_data.items():
    ...
    async def get_lab_topology(arguments: GetTopologyArgs) -> list[TextContent]:
        ...
        try:
            ...
            if not isinstance(topology, dict) or not topology.get('data'):
                return [TextContent(
                    type="text",
            ...
            if topology_data:
                ...
                if isinstance(topology_data, list):
                    ...
                    for connection_id, connection in topology_data.items():
                        if not isinstance(connection, dict):
                            topology_text += f"   {connection_id}: {connection}\n"
                            continue
                        src_type = "Node" if connection.get('source_type') == 'node' else "Network"
                        dst_type = "Node" if connection.get('destination_type') == 'node' else "Network"
                    ...
                    topology_text += f"   Unexpected data format: {type(topology_data)}\n"
            else:
