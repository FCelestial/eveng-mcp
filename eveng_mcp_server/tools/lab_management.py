...
def register_lab_tools(mcp: "FastMCP", eveng_client: "EVENGClientWrapper") -> None:
    ...
    async def get_lab_details(lab_path: str) -> list[TextContent]:
        ...
        try:
            ...
            # Extract lab data from the response (handle SDK returning non-dict)
            if isinstance(lab_response, dict):
                lab = lab_response.get('data', {})
            else:
                logger.warning(f"get_lab returned unexpected type: {type(lab_response)}")
                lab = {}
            ...
            try:
                ...
                if isinstance(nodes_response, dict):
                    ...
                    logger.warning(f"list_nodes returned unexpected type: {type(nodes_response)}")
                    nodes = {}
            except Exception as e:
            ...
            try:
                ...
                if isinstance(networks_response, dict):
                    ...
                    logger.warning(f"list_lab_networks returned unexpected type: {type(networks_response)}")
                    networks = {}
            except Exception as e:
            ...
            try:
                ...
                if isinstance(links_response, dict):
                    links = links_response.get('data', {})
                else:
                    logger.warning(f"list_lab_links returned unexpected type: {type(links_response)}")
                    links = {}
            except Exception as e:
                logger.warning(f"Failed to get links for lab {lab_path}: {e}")
                links = {}

            # Ensure data is always a dict (SDK may return list for some endpoints)
            ...
            for node_id in nodes.keys():
                try:
                    ...
                    node_interfaces[node_id] = interfaces_response.get('data', {}) if isinstance(interfaces_response, dict) else {}
                except Exception as e:
            ...
            # Nodes information
            details_text += f"🖥�? Nodes ({len(nodes)}):\n"
            if nodes:
