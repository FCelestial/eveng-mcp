...
class EVENGClientWrapper(LoggerMixin):
    ...
    def _patch_make_request(self) -> None:
        """Patch SDK's _make_request to always return a dict.

        The original SDK returns the raw Response object when JSON decoding
        fails, which causes 'str' object has no attribute 'get' errors
        downstream. This patch ensures a dict is always returned.
        """
        original = self._client._make_request
        wrapper_self = self

        def patched_make_request(method, url, use_prefix=True, *args, **kwargs):
            result = original(method, url, use_prefix, *args, **kwargs)
            # If SDK returned a Response object (JSON decode failed), try to
            # extract useful info or return an error dict
            if not isinstance(result, dict):
                if hasattr(result, 'text'):
                    wrapper_self.logger.warning(
                        f"SDK returned Response object instead of dict for {method} {url}",
                    ...
                    try:
                        ...
                        if isinstance(parsed, dict):
                            return parsed
                        # JSON decoded but is not a dict (e.g., a string or list)
                        return {"data": parsed}
                    except Exception:
                        return {"code": getattr(result, 'status_code', 0), "message": result.text}
                # Non-dict, non-Response value (e.g., a plain string or list)
                wrapper_self.logger.warning(
                    f"SDK returned {type(result).__name__} instead of dict for {method} {url}"
                )
                return {"data": result}
            return result

        self._client._make_request = patched_make_request
    ...
    async def connect(self) -> None:
        ...
        async with self._session_lock:
            ...
            try:
                ...
                # Patch _make_request to always return a dict (fix SDK bug where
                # JSONDecodeError causes it to return the raw Response object)
                self._patch_make_request()
